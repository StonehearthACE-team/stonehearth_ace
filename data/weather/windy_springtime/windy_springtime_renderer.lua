...
local WindySpringtimeWeatherRenderer = class()

local GUST_INTERVAL = {3, 6}  -- Random in range in game minutes
local GUST_EFFECTS = {
   { uri = 'stonehearth_ace:effects:wind_springtime:breeze_swirl', size = Point3(6, 5, 6) },
   { uri = 'stonehearth_ace:effects:wind_springtime:breeze', size = Point3(5, 4, 30) },
}
local MAX_POINTS_TO_TRY_PER_SPAWN = 3
local SCREEN_MARGIN = 100
local DEFAULT_CLOUD_SPEED = 1
local WINDY_CLOUD_SPEED = 2

-- TODO: Interpolate cloud speed changes.

function WindySpringtimeWeatherRenderer:initialize()
...
function WindySpringtimeWeatherRenderer:destroy()
...
function WindySpringtimeWeatherRenderer:activate()
...
function WindySpringtimeWeatherRenderer:_update_time(now)
...
function WindySpringtimeWeatherRenderer:_try_spawn_gust()
...
function WindySpringtimeWeatherRenderer:_spawn_gust_at(location, effect)
...
return WindySpringtimeWeatherRenderer
