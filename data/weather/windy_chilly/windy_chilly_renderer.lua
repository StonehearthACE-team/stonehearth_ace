...
local WindyChillyWeatherRenderer = class()

local GUST_INTERVAL = {3, 6}  -- Random in range in game minutes
local GUST_EFFECTS = {
   { uri = 'stonehearth_ace:effects:wind_chilly:breeze_swirl', size = Point3(6, 5, 6) },
   { uri = 'stonehearth_ace:effects:wind_chilly:breeze', size = Point3(5, 4, 30) },
}
local MAX_POINTS_TO_TRY_PER_SPAWN = 3
local SCREEN_MARGIN = 100
local DEFAULT_CLOUD_SPEED = 1
local WINDY_CLOUD_SPEED = 4

-- TODO: Interpolate cloud speed changes.

function WindyChillyWeatherRenderer:initialize()
...
function WindyChillyWeatherRenderer:destroy()
...
function WindyChillyWeatherRenderer:activate()
...
function WindyChillyWeatherRenderer:_update_time(now)
...
function WindyChillyWeatherRenderer:_try_spawn_gust()
...
function WindyChillyWeatherRenderer:_spawn_gust_at(location, effect)
...
return WindyChillyWeatherRenderer
