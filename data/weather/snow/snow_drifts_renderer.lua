...
local SnowDriftsWeatherRenderer = class()

local DRIFT_INTERVAL = {1, 3}  -- Random in range in game minutes
local DRIFT_EFFECTS = {
   { uri = 'stonehearth_ace:effects:snow_drifts:swirl', size = Point3(6, 5, 6) },
   { uri = 'stonehearth_ace:effects:snow_drifts:breeze', size = Point3(5, 4, 30) },
}
local MAX_POINTS_TO_TRY_PER_SPAWN = 3
local SCREEN_MARGIN = 100

function SnowDriftsWeatherRenderer:initialize()
...
function SnowDriftsWeatherRenderer:destroy()
...
function SnowDriftsWeatherRenderer:activate()
...
function SnowDriftsWeatherRenderer:_update_time(now)
...
function SnowDriftsWeatherRenderer:_try_spawn_drift()
...
function SnowDriftsWeatherRenderer:_spawn_drift_at(location, effect)
...
return SnowDriftsWeatherRenderer
