...
local function make_storage_filter_fn(entity, args_filter_fn, owner_player_id, ignore_leases)
...
function FindReachableStorageContainingBestEntityType:start_thinking(ai, entity, args)
   local storage_filter_fn = stonehearth.ai:filter_from_key('stonehearth:find_reachable_storage_containing_best_entity_type',
                                                            args.filter_fn, make_storage_filter_fn(entity, args.filter_fn, args.owner_player_id, args.ignore_leases))
   ai:set_think_output({
