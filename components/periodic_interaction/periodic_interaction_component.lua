--[[
    ...
    "stonehearth_ace:periodic_interaction": {
        ...
        "modes": {
            "find_normal_seed": {
                "ai_status_key": "stonehearth_ace:ai.actions.status_text.periodic_interaction.find_normal_seed.default",
                "ui_data": {
                    "name": "i18n(stonehearth_ace:entities.entity_name.periodic_interaction.modes.find_normal_seed.name)",
                    "description": "i18n(stonehearth_ace:entities.entity_name.periodic_interaction.modes.find_normal_seed.description)",
                },
                "cooldown": "12h",
                "requirements": {
                    ...
                    "quick": [
                        {
                            ...
                            "interaction_points": [
                                {
                                    "ai_status_key": "stonehearth_ace:ai.actions.status_text.periodic_interaction.find_normal_seed.specific_1",
...
function PeriodicInteractionComponent:get_current_interaction()
    return self._current_sequence and self._current_sequence[self._sv.interaction_stage]
end
...
function PeriodicInteractionComponent:set_current_interaction_completed(user)
    ...
    if self._sv.interaction_stage == 1 then
        self:_start_general_cooldown_timer(duration)
    end
...
function PeriodicInteractionComponent:_apply_reward(reward, is_completed)
    ...
    elseif reward.type == 'script' then
        local script = require(reward.script)
...
function PeriodicInteractionComponent:_consider_usability()
    -- if it's disabled or still on interaction cooldown, don't inform anyone
    if not self._sv.enabled or self._sv._interaction_cooldown_timer then
        return
    end
