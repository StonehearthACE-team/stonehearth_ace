...
function StatisticsComponent:increment_stat(category, name, value, default, sync)
   ...
   self:_trigger_on_changed(category, name, value, sync)
end

function StatisticsComponent:add_to_stat_list(category, name, value, default, sync)
...
function StatisticsComponent:_trigger_on_changed(category, name, value, sync)
   local args = {
      category = category,
      name = name,
      value = value
   ...
   radiant.events[sync and 'trigger' or 'trigger_async'](self._entity, 'stonehearth_ace:stat_changed', args)
end
