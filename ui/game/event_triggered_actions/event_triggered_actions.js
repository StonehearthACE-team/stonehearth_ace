$(document).ready(function(){
   ...
   $(top).on("stonehearth_ace_adjust_extensible_object", function (_, e) {
      ...
      adjustExtensibleObject(e.entity, e.event_data);
   });

   var adjustExtensibleObject = function(fromEntity, eventData) {
      App.stonehearthClient.showTipWithKeyBindings(eventData && eventData.show_tip_title,
                                                   eventData && eventData.show_tip_description,
                                                   { left_binding: 'build:rotate:left', right_binding: 'build:rotate:right',
